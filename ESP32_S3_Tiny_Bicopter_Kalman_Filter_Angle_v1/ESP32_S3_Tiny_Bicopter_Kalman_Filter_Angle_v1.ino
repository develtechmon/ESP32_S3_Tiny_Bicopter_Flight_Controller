...
#include <ESP32Servo.h>  // Add ESP32Servo library for servo control
...
// ===== Bicopter Motor and Servo Pins =====
#define LEFT_MOTOR_PIN 5   // Example: GPIO5 for left motor
#define RIGHT_MOTOR_PIN 6  // Example: GPIO6 for right motor
#define LEFT_SERVO_PIN 7   // Example: GPIO7 for left servo
#define RIGHT_SERVO_PIN 8  // Example: GPIO8 for right servo

// ===== Flight Controller / PID Declarations =====
uint32_t LoopTimer;
volatile float MotorInputLeft, MotorInputRight;  // Changed from 4 motors to 2
volatile float ServoInputLeft, ServoInputRight;  // Added for servo control
...
// PID gains (may need tuning for bicopter)
...
int ServoCenter = 90;   // Center position (degrees)
int ServoMin = 45;      // Minimum angle
int ServoMax = 135;     // Maximum angle
...
// Assign each servo to a unique LEDC channel
const int leftServoChannel = 3;
const int rightServoChannel = 4;
...
void setup() {
  ...
  ledcSetup(leftMotorChannel, pwmFrequency, pwmResolution);
  ledcSetup(rightMotorChannel, pwmFrequency, pwmResolution);
  ledcAttachPin(LEFT_MOTOR_PIN, leftMotorChannel);
  ledcAttachPin(RIGHT_MOTOR_PIN, rightMotorChannel);
  ...
  // ----- Setup Servos -----
  // Initialize servos using ESP32's LEDC for servo control
  ESP32PWM::allocateTimer(0);  // Allocate timers for ESP32Servo
  ESP32PWM::allocateTimer(1);
  
  leftServo.setPeriodHertz(50);  // Standard 50Hz servo frequency
  rightServo.setPeriodHertz(50);
  
  leftServo.attach(LEFT_SERVO_PIN, 500, 2400);  // Attach servos with min/max pulse width
  rightServo.attach(RIGHT_SERVO_PIN, 500, 2400);
  
  // Center the servos
  leftServo.write(ServoCenter);
  rightServo.write(ServoCenter);
...
void loop() {
  ...
  if (!armed) {
    ...
    ledcWrite(leftMotorChannel, pwmLeft);
    ledcWrite(rightMotorChannel, pwmRight);
  ...
  // ----- Bicopter Motor and Servo Mixing -----
  // Throttle affects both motors equally
  // Roll affects the difference between left and right motors
  MotorInputLeft = InputThrottle - InputRoll;
  MotorInputRight = InputThrottle + InputRoll;

  // Pitch and Yaw control the servos
  // For pitch: both servos tilt in the same direction
  // For yaw: servos tilt in opposite directions
  ...
  // Update servo positions
  leftServo.write(ServoInputLeft);
  rightServo.write(ServoInputRight);
