...
// Servo output pin
#define LEFT_SERVO_PIN 5          // Digital pin 5 
#define RIGHT_SERVO_PIN 6          // Digital pin 6 

volatile float ServoLeftInput, ServoRightInput;  // Two servos for bicopter

// Tilt limits
const int maxPitchDeg = 30;   // ±30° pitch
const int maxYawDeg   = 30;   // ±30° yaw
const int midAngle    = 90;   // vertical position
const int maxdeflection = 45; // maximum deflection from center

Servo servoLeft;   // 5) Create servo object
Servo servoRight;  // 6) Create servo object
...
uint32_t LoopTimer;
volatile float MotorInput1, MotorInput2;
...
void setup() {
  ...
  // ----- Setup Servos -----
  servoLeft.setPeriodHertz(50);    // Standard 50Hz servo frequency
  servoRight.setPeriodHertz(50);   // Standard 50Hz servo frequency
  
  // Attach servos to respective pins
  servoLeft.attach(LEFT_SERVO_PIN, 1000, 2000);  // min/max pulse width
  servoRight.attach(RIGHT_SERVO_PIN, 1000, 2000);  // min/max pulse width

  // Center the servos
  servoLeft.write(midAngle);  
  servoRight.write(midAngle);  
  // ########################################################
...
void loop() {
  ...
  if (!armed) {
    ...
    ServoLeftInput = ServoRightInput = midAngle;
    ...
    // -----Write servo to its default position -----
    servoLeft.write(midAngle);
    servoRight.write(midAngle);
  ...
  // ----- Bicopter Mixing For ESC Motor -----
  // Throttle and roll are handled by the two 8520 motors
  // Left motor: throttle + roll
  // Right motor: throttle - roll
  ...
  // ----- Bicopter Mixing For Servo Motor -----
  // Pitch and yaw are handled by the two servos
  // Servos are positioned to tilt the motors
  // Servo position is a combination of pitch and yaw commands
  // For mirrored servo configuration:
  ServoLeftInput = midAngle + InputPitch + InputYaw;
  ServoRightInput = midAngle + InputPitch - InputYaw;

  // ServoLeftInput = constrain(ServoLeftInput, midAngle - maxdeflection, midAngle + maxdeflection);
  // ServoRightInput = constrain(ServoRightInput, midAngle - maxdeflection, midAngle + maxdeflection);

  ServoLeftInput = constrain(ServoLeftInput, 0 ,180);
  ServoRightInput = constrain(ServoRightInput, 0 ,180);
  ...
  // Apply direction multipliers to account for servo mounting orientation
  servoLeft.write(ServoLeftInput);
  servoRight.write(ServoRightInput);
