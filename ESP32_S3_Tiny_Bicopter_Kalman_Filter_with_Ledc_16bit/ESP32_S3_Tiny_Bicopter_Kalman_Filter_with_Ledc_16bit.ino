...
uint32_t LoopTimer;
volatile float MotorInput1, MotorInput2; // Only two motors for bicopter
...
// PWM Configuration
const int pwmFrequency = 20000; // 20 kHz for motors
const int pwmResolution = 8;    // 8-bit resolution for motors

// Motor and Servo Channels
const int motor1Channel = 11;
const int motor2Channel = 10;
const int servo1Channel = 8;  // Repurposed from motor channels
const int servo2Channel = 9;

// Hardware Pins (example assignments, adjust as needed)
const int motor1Pin = 14;
const int motor2Pin = 15;
const int servo1Pin = 5;
const int servo2Pin = 6;

// Servo Scaling Factors (tune as needed)
const float k = 0.075; // Maps PID output to ±30 degrees at max 400
const float m = 0.075;
...
void setup() {
  ...
  // Setup PWM for Servos (50Hz, 16-bit resolution)
  ...
  int servoPulse = map(90, 0, 180, 1000, 2000); // Neutral position
  uint32_t servoDuty = (servoPulse * 13107UL) / 4000;
  ledcWrite(servo1Channel, servoDuty);
  ledcWrite(servo2Channel, servoDuty);
...
void loop() {
  ...
  if (!armed) {
    ...
    int servoPulse = map(90, 0, 180, 1000, 2000);
    uint32_t servoDuty = (servoPulse * 13107UL) / 4000;
    ledcWrite(servo1Channel, servoDuty);
    ledcWrite(servo2Channel, servoDuty);
  ...
  // Servo Mixing (mirrored setup)
  float servo1Angle = 90 + k * InputPitch + m * InputYaw; // Left servo
  float servo2Angle = 90 - k * InputPitch + m * InputYaw; // Right servo, mirrored
  servo1Angle = constrain(servo1Angle, 60, 120); // ±30 degrees from neutral
  servo2Angle = constrain(servo2Angle, 60, 120);

  // Convert to PWM
  int servo1Pulse = map(servo1Angle, 0, 180, 1000, 2000);
  int servo2Pulse = map(servo2Angle, 0, 180, 1000, 2000);
  uint32_t duty1 = (servo1Pulse * 13107UL) / 4000; // 16-bit duty for 50Hz
  uint32_t duty2 = (servo2Pulse * 13107UL) / 4000;
  ledcWrite(servo1Channel, duty1);
  ledcWrite(servo2Channel, duty2);
